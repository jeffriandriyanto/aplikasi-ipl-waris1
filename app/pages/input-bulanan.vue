    ...
    <template v-else>
      ...
      <div class="glass-card mb-4 sticky top-0 z-30 shadow-sm">
        <div class="p-3 space-y-3">
          <div class="flex gap-2">
            <select
              v-model="filterBlock"
              class="select-field py-2 text-sm flex-1"
            >
      ...
      <div class="mb-4 flex items-center justify-between">
        <p class="text-xs text-surface-500">
          {{ filteredRecords.length }} rumah
        </p>
        <p class="text-xs font-medium text-primary">
          Tagihan belum lunas: {{ countUnpaid }}
        </p>
      </div>
      ...
      <div class="md:hidden space-y-3 mb-4">
        ...
        >
          ...
          <div class="grid grid-cols-2 gap-2 mb-3">
            <div>
              <label class="text-[10px] text-surface-400 mb-0.5 block"
                >Status Rumah</label
              >
              <select
                v-model="r.status_rumah"
                class="select-field py-1.5 text-xs w-full"
                :class="{
            ...
            <div>
              ...
              <input
                ...
                type="number"
                min="0"
                class="input-field py-1.5 text-xs font-mono w-full"
                :class="{ 'border-rose-500 bg-rose-50': isError(r) }"
              />
              <p v-if="isError(r)" class="text-[10px] text-rose-600 mt-0.5">
                Tidak boleh kurang dari {{ r.water_meter_past }}
              </p>
            </div>
          </div>
          ...
          <div class="grid grid-cols-2 gap-2 pt-2 border-t border-surface-100">
            ...
            <div>
              ...
              >
              <p
                class="font-mono text-xs font-semibold py-1.5"
                :class="getOverpaymentClass(r)"
              >
              ...
              >
                ...
                <td>
                  <input
                    ...
                    type="number"
                    min="0"
                    class="input-field py-1.5 text-xs font-mono w-28"
                    :placeholder="String(calculateTotal(r, siteConfig))"
                  />
                </td>
                <td>
                  <span
                    ...
                    :class="getOverpaymentClass(r)"
                    >{{ formatCurrency(overpayment(r, siteConfig)) }}</span
                  >
                </td>
              </tr>
      ...
      <div
        class="fixed bottom-0 left-0 right-0 md:relative md:bottom-auto p-3 md:p-0 md:mt-6 md:pb-10 bg-white border-t md:border-0 border-surface-200 z-20"
      >
        <button
...
// Perbaikan: Hapus console.log yang mengganggu reaktivitas di dalam computed
const filteredRecords = computed(() => {
  ...
  return records.value.filter((r) => {
    ...
    if (query) {
      if (
        !matchHouseNumber(r.house_number, query) &&
...
const countUnpaid = computed(
  () =>
    filteredRecords.value
      .filter(
        (r) =>
          r.status_rumah === "Ditinggali" || r.status_rumah === "Disewakan",
      )
      .filter((r) => r.status_iuran !== "Terbayarkan").length,
);

// Perbaikan: Kunci tombol sync jika records kosong untuk menghindari post data kosong
...
function isError(r: IplRecord) {
  // Pastikan konversi tipe data aman saat komparasi nilai input tipe number
...
async function loadPeriod() {
  ...
  try {
    ...
    // Perbaikan logic inisialisasi amount_paid agar tidak mengunci nilai kalkulasi statis yang salah
    records.value = res.records.map((r) => {
      const currentRecord = { ...r };
      // Jika dari DB null/undefined, pasang default 0 atau sinkronkan secara dinamis melalui watch/event handler jika diperlukan
      currentRecord.amount_paid = r.amount_paid ?? calculateTotal(r, config);
