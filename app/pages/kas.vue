  ...
  <div class="page-container">
    ...
    <div v-if="showModal" class="modal-overlay" @click.self="closeModal">
      <div class="modal-content max-w-md">
        <h3 class="text-lg font-semibold text-surface-900 mb-4">Tambah Transaksi</h3>
        <form @submit.prevent="saveEntry" class="space-y-4">
          <div>
            <label class="label-field">Tipe</label>
            <select v-model="entryForm.type" class="select-field text-sm" required>
          ...
          <div>
            ...
            <select v-model="entryForm.category" class="select-field text-sm" required>
          ...
          <div>
            ...
            <input v-model="entryForm.description" type="text" class="input-field text-sm" placeholder="Contoh: Sumbangan RT 01" required />
          </div>
          <div>
            <label class="label-field">Jumlah (Rp)</label>
            <input v-model.number="entryForm.amount" type="number" min="1" class="input-field text-sm" placeholder="50000" required />
          </div>
          <div class="flex gap-3 pt-2">
            <button type="button" class="btn-secondary flex-1" @click="closeModal">Batal</button>
            <button type="submit" class="btn-primary flex-1" :disabled="isSaving">Simpan</button>
          </div>
        </form>
      </div>
    </div>
    ...
    <div v-if="showDeleteConfirm" class="modal-overlay" @click.self="showDeleteConfirm = false">
      <div class="modal-content max-w-sm">
        <h3 class="text-lg font-semibold text-surface-900 mb-2">Hapus Transaksi?</h3>
        <p class="text-sm text-surface-500 mb-6">Transaksi ini akan dihapus secara permanen.</p>
        <div class="flex gap-3">
          <button class="btn-secondary flex-1" @click="showDeleteConfirm = false">Batal</button>
          <button class="btn-danger flex-1" @click="deleteEntry" :disabled="isDeleting">Hapus</button>
        </div>
      </div>
    </div>
    ...
    <div class="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4 mb-6">
      <div>
        ...
        <p class="page-subtitle">Kelola kas masuk, kas keluar, dan laporan keuangan</p>
      </div>
    ...
    <div v-else-if="!hasLoaded" class="flex flex-col items-center justify-center py-32">
    ...
    <template v-else>
      <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4 mb-6">
        <div class="stat-card group">
          <div class="flex items-start justify-between mb-3">
            <div class="w-10 h-10 rounded-xl bg-primary/10 flex items-center justify-center group-hover:scale-110 transition-transform duration-300">
          ...
          <p class="text-xl font-bold text-primary mb-1">{{ formatCurrency(summary.totalIuran) }}</p>
        ...
        <div class="stat-card group">
          <div class="flex items-start justify-between mb-3">
            ...
            <span class="badge bg-emerald-50 text-emerald-600 text-[10px]">Masuk</span>
          </div>
          <p class="text-xl font-bold text-emerald-600 mb-1">{{ formatCurrency(summary.kasMasukManual) }}</p>
          <p class="text-xs text-surface-400">Kas masuk manual</p>
        </div>

        <div class="stat-card group">
          <div class="flex items-start justify-between mb-3">
            <div class="w-10 h-10 rounded-xl bg-rose-500/10 flex items-center justify-center group-hover:scale-110 transition-transform duration-300">
              <svg class="w-5 h-5 text-rose-600" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M20 12H4" />
              </svg>
            </div>
            <span class="badge bg-rose-50 text-rose-600 text-[10px]">Keluar</span>
          </div>
          <p class="text-xl font-bold text-rose-600 mb-1">{{ formatCurrency(summary.kasKeluar) }}</p>
        ...
        <div class="stat-card group">
          <div class="flex items-start justify-between mb-3">
            ...
            <span class="badge bg-amber-50 text-amber-600 text-[10px]">Saldo</span>
          </div>
          <p class="text-xl font-bold mb-1" :class="summary.netSaldo >= 0 ? 'text-primary' : 'text-rose-600'">
      ...
      <div class="glass-card overflow-hidden mb-6" id="export-target">
        <div class="px-6 py-4 border-b border-surface-200 flex items-center justify-between flex-wrap gap-3">
          <div>
            <h2 class="text-lg font-semibold text-surface-900">Riwayat Transaksi</h2>
            <p class="text-xs text-surface-500 mt-0.5">{{ kasEntries.length }} transaksi</p>
          </div>
          <div class="flex items-center gap-2">
            ...
            <button class="btn-ghost text-xs" @click="exportPDF">
              <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
        ...
        <div v-if="kasEntries.length === 0" class="flex flex-col items-center justify-center py-16 px-4">
          ...
          <p class="text-surface-400 text-xs mt-1">Klik "Tambah" untuk menambah transaksi.</p>
        </div>
          ...
          <table class="data-table">
            ...
            <tbody>
              <tr v-for="(entry, i) in kasEntries" :key="entry.id">
                ...
                <td>
                  <span :class="entry.type === 'masuk' ? 'badge-paid' : 'badge-unpaid'" class="text-[10px]">
                    {{ entry.type === 'masuk' ? 'Masuk' : 'Keluar' }}
                  </span>
                </td>
                <td class="text-surface-700">{{ entry.category }}</td>
                <td class="text-surface-700">{{ entry.description }}</td>
                <td class="font-mono font-semibold" :class="entry.type === 'masuk' ? 'text-emerald-600' : 'text-rose-600'">
                  {{ entry.type === 'masuk' ? '+' : '-' }}{{ formatCurrency(entry.amount) }}
                </td>
                <td>
                  <button class="btn-ghost text-xs text-rose-500 hover:text-rose-700 hover:bg-rose-50" @click="confirmDelete(entry)">
                    <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
...
watch(() => entryForm.type, () => {
  entryForm.category = ''
})

const summary = computed(() => {
  const paidRecords = iplRecords.value.filter((r) => r.status_iuran === 'Terbayarkan')
  const totalIuran = paidRecords.reduce((sum, r) => sum + calculateTotal(r, siteConfig.value), 0)
...
async function saveEntry() {
  if (!entryForm.category || !entryForm.description || entryForm.amount <= 0) return
  isSaving.value = true
  try {
...
async function deleteEntry() {
  ...
  try {
    ...
    kasEntries.value = kasEntries.value.filter((e) => e.id !== deleteTarget.value!.id)
...
function exportCSV() {
  const headers = ['Tipe', 'Kategori', 'Deskripsi', 'Jumlah (Rp)']
  ...
  const summaryRow2 = ['', '', 'Total Kas Masuk Manual', summary.value.kasMasukManual]
  const summaryRow3 = ['', '', 'Total Kas Keluar', summary.value.kasKeluar]
  const summaryRow4 = ['', '', 'Saldo Bersih', summary.value.netSaldo]

  const csv = [headers, ...rows, [], summaryRow, summaryRow2, summaryRow3, summaryRow4]
    .map((row) => row.join(','))
    .join('\n')
...
async function exportPDF() {
  if (kasEntries.value.length > 100) {
  ...
  html2pdf()
    .set({
      margin: 10,
      filename: `Kas_${selectedPeriod.value}.pdf`,
      html2canvas: { scale: 1 },
