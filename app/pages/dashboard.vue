    ...
    <div class="glass-card overflow-hidden" id="dashboard-export-target">
      ...
      >
        ...
        <div class="flex items-center gap-2">
          ...
          <button
            id="refresh-data"
            class="bg-primary text-white flex items-center w-[190px] px-4 py-2 rounded gap-2 text-sm"
            @click="loadData"
            :disabled="isLoading"
          >
            <svg
      ...
      >
        ...
        <p class="text-surface-600 font-medium mb-1">
          Pilih periode dan klik "Muat Data"
        </p>
        <p class="text-surface-400 text-sm text-center">
          Data hanya dimuat saat diminta untuk menghemat kuota pembacaan
          database.
        </p>
      </div>
        ...
        <table class="data-table">
          ...
          <tbody>
            <tr v-for="(r, i) in filteredRecords" :key="r.id || i">
              ...
              <td class="font-mono text-surface-900">
...
async function exportPDF() {
  if (filteredRecords.value.length > 100) {
    toast.show(
      "Data terlalu besar untuk PDF. Gunakan CSV untuk data >100 baris.",
      "info",
    );
    return;
  }
